...
def vp_start_gui():
    ...
    root = tk.Tk()
    global top
    top = Modify_Toplevel(root)
    modify_support.init(root, top)
    root.mainloop()

w = None
def create_Modify_Toplevel(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = tk.Toplevel (root)
    top = Modify_Toplevel (w)
    modify_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_Modify_Toplevel():
    global w
    w.destroy()
    w = None

class Modify_Toplevel:
    def __init__(self, top=None):
        ...
        top.title("Modify")
        top.configure(background="#a591a3")
        ...
        modify_ok_handler = partial(modify_support.modify_ok_handler, self)

        self.OK_Button.configure(command=modify_ok_handler)
        ...
        modify_cancel_handler = partial(modify_support.modify_cancel_handler, self, top)

        self.Cancel_Button.configure(command=modify_cancel_handler)
