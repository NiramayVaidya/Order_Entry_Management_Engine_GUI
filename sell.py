...
def vp_start_gui():
    ...
    root = tk.Tk()
    global top
    top = Sell_Toplevel(root)
    sell_support.init(root, top)
    root.mainloop()

w = None
def create_Sell_Toplevel(root, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = tk.Toplevel (root)
    top = Sell_Toplevel (w)
    sell_support.init(w, top, *args, **kwargs)
    return (w, top)

def destroy_Sell_Toplevel():
    global w
    w.destroy()
    w = None
...
class Sell_Toplevel:
    def __init__(self, top=None):
        ...
        top.title("Sell")
        top.configure(background="#b21227")
        ...
        sell_ok_handler = partial(sell_support.sell_ok_handler, self, top)

        self.OK_Button.configure(command=sell_ok_handler)
        ...
        sell_cancel_handler = partial(sell_support.sell_cancel_handler, self, top)

        self.Cancel_Button.configure(command=sell_cancel_handler)
